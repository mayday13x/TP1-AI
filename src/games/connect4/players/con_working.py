...
class teste(Connect4Player):

    def __init__(self, name):
        super().__init__(name)

    def get_action(self, state: Connect4State):
        col, value = self.minimax(state, 4, True)
    ...
    def minimax(self, state: Connect4State, depth: int , max_player: bool):
        ...
        if max_player:
            ...
            for action in state.get_possible_actions():
                ...
                new_value = self.minimax(state_clone, depth - 1, False)[1]
                if new_value > value:
                    value = new_value
            ...
            for action in state.get_possible_actions():
                state_clone = state.clone()
                state_clone.update(action)
                new_value = self.minimax(state_clone, depth - 1, True)[1]
                if new_value < value:
                    value = new_value
                    best_col = action.get_col()
                '''if not new_value is None:
